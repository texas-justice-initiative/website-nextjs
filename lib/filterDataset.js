...
export default function filterData(records, filters) {
    ...
    if (applyFilters) {
      ...
      groupOptions.forEach((groupOption) => {
        if (filters[filterGroup][groupOption] === false) {
          ...
          const matchedRecords = filteredData.records[filterGroup].reduce(
            (acc, curr, index) => {
              ...
              const currentOption =
                typeof curr === 'boolean' ? curr.toString() : curr;
  ...
  if (uniqueFilters.length > 0) {
    ...
    filterGroups.forEach((filterGroup) => {
      ...
      cleanedData.records[filterGroup] = filteredData.records[
        filterGroup
